...
## External URL Confirmation and Redirect Warning

### 目的
- 外部 URL クリック時に確認ダイアログが表示されることを確認する。
- `HEAD` 結果がリダイレクトの場合、`Location` を含む警告が表示されることを確認する。

### 前提
- テスト用 Markdown (`external.md`) に以下 2 つのリンクを用意する:
  - リダイレクトあり URL（例: 短縮 URL）
  - リダイレクトなし URL（例: 直接 URL）

### 手順
1. `external.md` を mdprev で開く。
2. リダイレクトなし URL をクリックする。
3. 表示されたダイアログで URL が提示されることを確認し、`Cancel` を選択する。
4. 同じ URL を再度クリックし、今度は `Open` を選択する。
5. リダイレクトあり URL をクリックする。
6. ダイアログにリダイレクト警告と `Location` の内容が表示されることを確認する。
7. `Cancel` と `Open` の両方を試す。

### 期待結果
- 常に確認ダイアログが表示される。
- リダイレクトあり URL では `Location` を含む警告文が表示される。
- `Cancel` 時は外部ブラウザが起動しない。
- `Open` 時は `open` コマンド経由で既定ブラウザが起動する。
